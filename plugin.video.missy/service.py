...
class background_queue():
    @classmethod
    def put(cls, item):
        xbmcgui.Window(10008).setProperty('Missy_Queue', repr(item))

    @classmethod
    def get(cls):
        try:
            item = xbmc.getInfoLabel('Window(10008).Property(Missy_Queue)')
            xbmcgui.Window(10008).clearProperty('Missy_Queue')
            return item
        except Exception:
            return ""
    ...
    if "<item>" in str(missy_item):
        ...
        if resolved:
            playlist.add(resolved,
                         xbmcgui.ListItem(missy_item["title"],
                                          iconImage=missy_item.get("thumbnail",
                                                                 "")))
        if play:
