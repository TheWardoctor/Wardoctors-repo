...
from resources.lib.util.xml import MissyItem, MissyList
...
from resources.lib.player import MissyPlayer
...
def choose_quality(link, name=None, selected_link=None):
    ...
    Keyword Arguments:
    link -- Missyitem link with sublinks
    ...
    if link.startswith("http") or link.startswith("plugin"):
        ...
        missy_link = MissyItem(link)
        sublinks = missy_link.getAll("sublink")
        if not sublinks:
            sublinks = [missy_link]
    links = []
...
def get_sources(item):
    ...
    Keyword Arguments:
    item -- MissyItem to try playing
    """
    if item.startswith("<plugin>"):
        # link to plugin
        link = MissyItem(item)["link"]
        sublinks = MissyItem(link).getAll("sublink")
        if sublinks:
            if len(sublinks) > 1:
    ...
    missyplayer = MissyPlayer(resume=False)
    try:
        spec = {
            "identifier": imdb,
            "season": season or "0",
        ...
        if match:
            ...
            if match["currentTime"] and not match["currentTime"] == "0":
                if dialog.yesno(ADDON.getAddonInfo("name"),
                                ...
                                yeslabel="Resume",
                                nolabel="Restart"):
                    missyplayer = MissyPlayer(resume=True)
    except:
        pass

    missyplayer.setItem(item)
...
def queue_source(item, depth=0):
    ...
    missy_item = MissyItem(item)
    playlist = xbmc.PlayList(xbmc.PLAYLIST_VIDEO)
    if xbmcaddon.Addon().getSetting("background_list_queue") == "true":
        item = xbmc.getInfoLabel('Window(10008).Property(Missy_Queue)')
        if item:
            xbmcgui.Dialog().ok(
                "still queueing last one, please try again later")
            return
        import service
        service.background_queue.put(missy_item)
        return
    if "<item>" in str(missy_item):
        play = False
        ...
        playlist.add(
            ...
            xbmcgui.ListItem(
                missy_item["title"], iconImage=missy_item.get("thumbnail", "")))
        if play:
            play_queue()
    else:
        link = missy_item.get("url", missy_item.get("link", ""))
        missylist = MissyList(link).get_raw_list()
        for list_item in missylist:
