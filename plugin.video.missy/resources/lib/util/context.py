...
"""
    context.py --- functions to generate a context menu for missy items
...
def get_context_items(item):
    ...
    if xbmc.getCondVisibility("system.hasaddon(plugin.video.metalliq)"):
        ...
        if imdb.startswith("tt"):
            if content == "movie":
                path = "plugin://plugin.video.metalliq/movies/add_to_library_parsed/imdb/%s/direct.missy.unleashed.m" % imdb
                context.append(("Add Movie To Library",
                                "RunPlugin({0})".format(path)))
            elif content in ["tvshow", "season", "episode"]:
                path = "plugin://plugin.video.metalliq/tv/add_to_library_parsed/%s/direct.missy.unleashed.m" % imdb
