...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
    ...
    def onClick(self, controlId):
        if controlId == self.C_STREAM_BROWSE_ADDONS:
            ...
            dialog = xbmcgui.Dialog()
            stream = dialog.browse(1, ADDON.getLocalizedString(30304), 'video', '.strm|.m3u|.m3u8|.xsp')
            if stream:
                stream_name = stream
            ...
            dialog = xbmcgui.Dialog()
            stream = dialog.browseSingle(1, ADDON.getLocalizedString(30304), 'files', '.strm|.m3u|.m3u8|.xsp')
            if stream:
                stream_name = stream
                f = xbmcvfs.File(stream,"rb")
                data = f.read()
