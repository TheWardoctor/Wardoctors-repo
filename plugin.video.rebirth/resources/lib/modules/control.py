...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.rebirth.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.rebirth.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.rebirth.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.rebirth.artwork)')
...
def moderator():
    netloc = [urlparse.urlparse(sys.argv[0]).netloc, '', 'plugin.video.live.streamspro', 'plugin.video.phstreams', 'plugin.video.cpstreams', 'plugin.video.tinklepad', 'script.tvguide.fullscreen', 'script.tvguide.assassins', 'plugin.video.metalliq', 'plugin.video.missy', 'script.extendedinfo' ,'script.stargate.guide', 'plugin.program.simple.favourites', 'plugin.program.super.favourites']

    if not infoLabel('Container.PluginName') in netloc: sys.exit()


def metaFile():
    if condVisibility('System.HasAddon(script.rebirth.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.rebirth.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
