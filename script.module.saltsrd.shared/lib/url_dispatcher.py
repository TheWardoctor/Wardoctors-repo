"""
    ...
    Copyright (C) 2016 credits -2- tknorris
...
class URL_Dispatcher:
    ...
    def dispatch(self, mode, queries):
        ...
        if unused_args: logger.log('Warning: Arguments |%s| were passed but unused by |%s| for mode |%s|' % (unused_args, self.func_registry[mode].__name__, mode), log_utils.LOGWARNING)
