            ...
            if selection == 0:
                if self.type == "TVShow":
                    if dbid:
                        ...
                        xbmc.executebuiltin("ActivateWindow(10025,plugin://plugin.video.metalliq/tv/tvdb/%s,return)" % tvdb)
                        self.close()
                else:
                    if dbid: url = "temp"
                    else:
                        dbid = 0
                        url = "plugin://plugin.video.metalliq/movies/play/tmdb/%s/%s" % (item_id, SETTING("player_main_movie"))
