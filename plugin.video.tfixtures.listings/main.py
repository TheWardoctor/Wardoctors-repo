...
def clear_addon_paths():
    ...
    dialog.notification("Tardis Fixtures Listings","Done: Clear Addon Paths")
...
def clear_addons():
    ...
    create_database_tables()
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Clear Addons")
...
def clear_channels():
    ...
    create_database_tables()
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Clear Channels")
...
def export_channels():
    ...
    if not folder:
        folder = 'special://profile/addon_data/plugin.video.tfixtures.listings'
    file_name = os.path.join(xbmc.translatePath(folder),'plugin.video.tfixtures.listings.ini')
    f = xbmcvfs.File(file_name,'w')
    if not f:
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Error: could not create file")
    write_str = "# WARNING Make a copy of this file.\n# It will be overwritten on the next channel export.\n\n[plugin.video.tfixtures.listings]\n"
    ...
    for addon in addons:
        ...
        for row in c:
            ...
            f.write(write_str.encode("utf8"))
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Export Channels")
    c.close()
    return items
...
def clear_reminders():
    ...
    conn.close()
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Clear Reminders")
...
def refresh_reminders():
    ...
    try:
        ...
        for row in c:
            ...
            t = datetime.fromtimestamp(float(start)) - datetime.now()
            timeToNotification = ((t.days * 86400) + t.seconds) / 60
            command = 'AlarmClock(%s-start,PlayMedia(plugin://plugin.video.tfixtures.listings/play_channel/%s/%s/%s),%d,False)' % (
            ...
            if plugin.get_setting('watch_and_stop') == 'true':
                ...
                timeToNotification = ((t.days * 86400) + t.seconds) / 60
                command = 'AlarmClock(%s-stop,PlayMedia(plugin://plugin.video.tfixtures.listings/stop_playing/%s/%s/%s),%d,True)' % (
    ...
    #if notify:
    #    dialog = xbmcgui.Dialog()
    #   dialog.notification("Tardis Fixtures Listings","Done: Refresh Reminders",sound=False)
...
def remind(channel_id,channel_name,title,season,episode,start,stop):
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Remind")
...
def watch(channel_id,channel_name,title,season,episode,start,stop):
    t = datetime.fromtimestamp(float(start)) - datetime.now()
    timeToNotification = ((t.days * 86400) + t.seconds) / 60
    xbmc.executebuiltin('AlarmClock(%s-start,PlayMedia(plugin://plugin.video.tfixtures.listings/play_channel/%s/%s/%s),%d,False)' %
        (channel_id+title+str(start), channel_id, title, start, timeToNotification - int(plugin.get_setting('remind_before'))))
    ...
    if plugin.get_setting('watch_and_stop') == 'true':
        t = datetime.fromtimestamp(float(stop)) - datetime.now()
        timeToNotification = ((t.days * 86400) + t.seconds) / 60
        xbmc.executebuiltin('AlarmClock(%s-stop,PlayMedia(plugin://plugin.video.tfixtures.listings/stop_playing/%s/%s/%s),%d,True)' %
            (channel_id+title+str(start), channel_id, title, start, timeToNotification + int(plugin.get_setting('remind_after'))))
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Watch")
...
def cancel_remind(channel_id,channel_name,title,season,episode,start,stop):
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Cancel Remind")
...
def cancel_watch(channel_id,channel_name,title,season,episode,start,stop):
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Cancel Watch")
...
def addon_streams_to_channels(addon_id):
    ...
    conn.close()
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Addon Shortcuts to Default Shortcuts")
...
def stream_remap(stream_name,path,icon):
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Update Shortcut")
...
def store_channels():
    ...
    if plugin.get_setting('ini_type') == '2':
        ...
        r = requests.get(url)
        file_name = 'special://profile/addon_data/plugin.video.tfixtures.listings/addons.ini'
        ...
        if not xbmcvfs.exists(path):
            dialog = xbmcgui.Dialog()
            dialog.notification("Tardis Fixtures Listings","Error: ini File Not Found!")
            return
    ...
    conn.close()
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Load ini File")
...
def xml_channels():
        ...
        if xmltv_type == xmltv_type_last:
            if plugin.get_setting('xmltv_type') == '0': # File
                if plugin.get_setting('xml_reload_modified') == 'true':
                    ...
                    if not xbmcvfs.exists(path):
                        dialog = xbmcgui.Dialog()
                        dialog.notification("Tardis Fixtures Listings","Error: xmltv File Not Found!")
                        return
                    stat = xbmcvfs.Stat(path)
                    modified = str(stat.st_mtime())
    ...
    xbmcvfs.mkdir('special://profile/addon_data/plugin.video.tfixtures.listings')
    ...
    dialog.notification("Tardis Fixtures Listings","downloading xmltv file")
    if plugin.get_setting('xmltv_type') == '1':
        url = plugin.get_setting('xmltv_url')
        r = requests.get(url)
        file_name = 'special://profile/addon_data/plugin.video.tfixtures.listings/xmltv.xml'
        xmltv_f = xbmcvfs.File(file_name,'w')
        xml = r.content
        if r.status_code == 404:
            dialog.notification("Tardis Fixtures Listings","ERROR: No xmltv Url Data")
            return
        xmltv_f.write(xml)
        xmltv_f.close()
    ...
    dialog.notification("Tardis Fixtures Listings","finished downloading xmltv file")
        ...
        if event == "end":
            ...
            if elem.tag == "channel":
                ...
                if (now - last).seconds > 0.5:
                    dialog.notification("Tardis Fixtures Listings","loading channels: "+display_name)
                    last = now
                ...
                if (now - last).seconds > 0.5:
                    dialog.notification("Tardis Fixtures Listings","loading programmes: "+channel)
                    last = now
            root.clear()
    ...
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Load xmltv File")
...
def nuke():
    TARGETFOLDER = xbmc.translatePath(
        'special://profile/addon_data/plugin.video.tfixtures.listings'
        )
    dialog = xbmcgui.Dialog()
    ok = dialog.ok('Tardis Fixtures Listings', '[COLOR crimson][B]Delete Everything in addon_data Folder?![/B][/COLOR]')
    if not ok:
        return
    if os.path.exists( TARGETFOLDER ):
            shutil.rmtree( TARGETFOLDER , ignore_errors=True)

    dialog.notification("Tardis Fixtures Listings","Done: Everything Deleted!")
...
def remove_addon_path(path):
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Removed")
...
def reload_addon_paths():
    ...
    dialog = xbmcgui.Dialog()
    dialog.notification("Tardis Fixtures Listings","Done: Addon Paths Refreshed")
...
def add_addon_channels(addon,path,path_name):
    ...
    if plugin.get_setting("refresh") == 'true':
        ...
        dialog = xbmcgui.Dialog()
        dialog.notification("Tardis Fixtures Listings","Done: Added")
