...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.master-reborn.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.master-reborn.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.master-reborn.artwork)') else setting('appearance.alt')
    if appearance == "Master Reborn": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.master-reborn.artwork)')
