...
from resources.libs import extract, downloader, notify, debridit, traktit, loginit, apiit, skinSwitch, uploadLog, yt, wizard as wiz
...
INCLUDESPECTO    = wiz.getS('includegenesisreborn')
...
INCLUDEsaltsrd   = wiz.getS('includesaltslite')
...
ICONAPI          = uservar.ICONAPI if not uservar.ICONAPI == 'http://' else ICON
...
def maintMenu(view=None):
	...
	if includeall == 'true':
		...
		includeshd = 'true' if INCLUDESALTSRD == 'true' else 'false'
	...
	if includevid == 'true':
		...
		addFile('--- Include Specto: %s' % includespe.replace('true',on).replace('false',off), 'togglecache', 'includegenesisreborn', icon=ICONMAINT, themeit=THEME3)
...
def saveMenu():
	...
	api        = 'true' if KEEPAPI      == 'true' else 'false'
	...
	addDir ('Keep API Data',				 'api',                  icon=ICONAPI,   themeit=THEME1)
	...
	addFile('Save API Data: %s' % api.replace('true',on).replace('false',off)                      ,'togglesetting', 'keepapi',       icon=ICONAPI,
	themeit=THEME1)
...
def apiMenu():
	api = '[COLOR green]ON[/COLOR]' if KEEPAPI == 'true' else '[COLOR red]OFF[/COLOR]'
	last = str(APISAVE) if not APISAVE == '' else 'API Data hasnt been saved yet.'
	addFile('[I]Save additional API Data[/I]', '', icon=ICONAPI, themeit=THEME3)
	addFile('Save Addon Data: %s' % api, 'togglesetting', 'keepapi', icon=ICONAPI, themeit=THEME3)
	if KEEPAPI == 'true': addFile('Last Save: %s' % str(last), '', icon=ICONAPI, themeit=THEME3)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', icon=ICONAPI, themeit=THEME3)
	
	for api in apiit.ORDER:
		name   = APIID[api]['name']
		path   = APIID[api]['path']
		saved  = APIID[api]['saved']
		file   = APIID[api]['file']
		user   = wiz.getS(saved)
		auser  = apiit.apiUser(api)
		icon   = APIID[api]['icon']   if os.path.exists(path) else ICONAPI
		fanart = APIID[api]['fanart'] if os.path.exists(path) else FANART
		menu = createMenu('saveaddon', 'api', api)
		menu2 = createMenu('save', 'api', api)
		menu.append((THEME2 % '%s Settings' % name,              'RunPlugin(plugin://%s/?mode=opensettings&name=%s&url=api)' %   (ADDON_ID, api)))
		
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR red]API Data: Not Installed[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR red]API Data: Not Registered[/COLOR]','authapi', api, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR green]API Data: %s[/COLOR]' % auser,'authapi', api, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR red]Saved Data: Save File Found(Import Data)[/COLOR]','importdata', api, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR red]Saved Data: Not Saved[/COLOR]','saveapi', api, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR green]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Save All API Data',          'saveapi',    'all', icon=ICONAPI,  themeit=THEME3)
	addFile('Recover All Saved API Data', 'restoreapi', 'all', icon=ICONAPI,  themeit=THEME3)
	addFile('Import API Data',            'importapi',  'all', icon=ICONAPI,  themeit=THEME3)
	addFile('Clear All Saved API Data',   'clearapi',   'all', icon=ICONAPI,  themeit=THEME3)
	addFile('Clear All API Data',         'addonapi',   'all', icon=ICONAPI,  themeit=THEME3)
	setView('files', 'viewType')
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includebob', 'includephoenix', 'includegenesisreborn', 'includegenesis', 'includeexodus', 'includeonechan', 'includesalts', 'includesaltslite']
...
def manageSaveData(do):
	if do == 'import':
		...
		if os.path.exists(data):
			...
			if not os.path.exists(apiit.APIFOLD): os.makedirs(apiit.APIFOLD)
			for item in files:
				old  = os.path.join(apiit.APIFOLD, item)
				temp = os.path.join(data, item)
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like replace the current [COLOR %s]%s[/COLOR] file?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR green]Yes Replace[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"): continue
					else: os.remove(old)
				shutil.copy(temp, old)
			apiit.importlist('all')
			apiit.apiIt('restore', 'all')
		wiz.cleanHouse(TEMP)
		...
		dir = [traktit.TRAKTFOLD, debridit.REALFOLD, loginit.LOGINFOLD, apiit.APIFOLD]
		traktit.traktIt('update', 'all')
		loginit.loginIt('update', 'all')
		debridit.debridIt('update', 'all')
		apiit.apiIt('update', 'all')
...
elif mode=='api'           : apiMenu()
elif mode=='saveapi'       : apiit.apiIt('update',      name)
elif mode=='restoreapi'    : apiit.apiIt('restore',     name)
elif mode=='addonapi'      : apiit.apiIt('clearaddon',  name)
elif mode=='clearapi'      : apiit.clearSaved(name)
elif mode=='authapi'       : apiit.activateApi(name); wiz.refresh()
elif mode=='updateapi'     : apiit.autoUpdate('all')
elif mode=='importapi'     : apiit.importlist(name); wiz.refresh()
