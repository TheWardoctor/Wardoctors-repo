...
from resources.libs import extract, downloader, notify, loginit, debridit, traktit, apiit, skinSwitch, uploadLog, wizard as wiz
...
if INSTALLED == 'true':
	...
	if KEEPAPI  == 'true': apiit.apiIt('restore', 'all'); wiz.log('[Installed Check] Restoring Addon Data', xbmc.LOGNOTICE)
	wiz.clearS('install')
...
wiz.log("[API Data] Started", xbmc.LOGNOTICE)
if KEEPAPI == 'true':
	if APISAVE <= str(TODAY):
		wiz.log("[PI Data] Saving all Data", xbmc.LOGNOTICE)
		apiit.autoUpdate('all')
		wiz.setS('apilastsave', str(THREEDAYS))
	else: 
		wiz.log("[API Data] Next Auto Save isnt until: %s / TODAY is: %s" % (APISAVE, str(TODAY)), xbmc.LOGNOTICE)
else: wiz.log("[API Data] Not Enabled", xbmc.LOGNOTICE)
