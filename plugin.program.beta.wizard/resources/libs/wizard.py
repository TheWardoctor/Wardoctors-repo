...
USER_AGENT     = 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.153 Safari/537.36 SE 2.X RebirthSr 1.0'
...
INCLUDESPECTO  = ADDON.getSetting('includegenesisreborn')
...
INCLUDEsaltsrd = ADDON.getSetting('includesaltslite')
...
def getCacheSize():
	...
	dbfiles   = [
		...
		(os.path.join(ADDONDATA, 'plugin.video.genesisreborn', 'cache.db')),
	...
	for item in cachelist:
		if os.path.exists(item) and not item in [ADDONDATA, PROFILEADDONDATA]:
			...
			for root, dirs, files in os.walk(item):
				for d in dirs:
					if 'cache' in d.lower() and not d.lower() == 'rebirth_cache': totalsize = getSize(os.path.join(root, d), totalsize)
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDESPECTO == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.genesisreborn', 'cache.db'))
...
def fixrebirths():
	idlist = ['plugin.video.metalliq', 'plugin.video.rebirth', 'script.renegadesrebirth']
	temp   = os.path.join(PLUGIN, 'resources', 'tempaddon.xml')
	f      = open(temp, 'r'); r = f.read(); f.close()
	for item in idlist:
		fold = os.path.join(ADDONS, item)
...
def backUpOptions(type, name=""):
	...
	bad_files     = [os.path.join(DATABASE, 'onechannelcache.db'),
					 ...
					 os.path.join(ADDOND, 'script.module.rebirthhandler', 'rebirth_cache', 'video_cache.db')]
...
def clearCache(over=None):
	...
	delfiles = 0
	excludes = ['rebirth_cache', 'archive_cache']
	for item in cachelist:
		if os.path.exists(item) and not item in [ADDONDATA, PROFILEADDONDATA]:
			for root, dirs, files in os.walk(item):
				dirs[:] = [d for d in dirs if d not in excludes]
