...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cDovLzE2My4xNzIuNDYuMTIyL2ZpbGVzL21hc3RlcmJldGEvbWFpbi54bWw=')
            self.list = self.master_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.master_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.master_list('', result=result)
    ...
    def addSearch(self, url):
            ...
            link = 'http://163.172.46.122/files/master/main/mastersearch.xml'
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.master_list, link))
            ...
            for i in self.list:
                try:
                    name = ''
                    if not i['vip'] in ['Master TV']: name += '[B]%s[/B] | ' % i['vip'].upper()
                    name += i['name']
                    i.update({'name' : name})
                except:
                    pass
    ...
    def master_list(self, url, result=None):
...
class resolver:
    ...
    def process(self, url, direct=True):
        ...
        try:
            if not any(i in url for i in ['.JPG', '.PNG', '.GIF']): raise Exception()
            ext = url.split('?')[0].split('&')[0].split('|')[0].rsplit('.')[-1].replace('/', '').upper()
            if not ext in ['JPG', 'PNG', 'GIF']: raise Exception()
            try:
                i = os.path.join(control.dataPath,'img')
                control.deleteFile(i)
