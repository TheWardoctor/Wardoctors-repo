...
class libmovies:
    ...
    def strmFile(self, i):
        try:
            ...
            content = '%s?action=playp&name=%s&title=%s&year=%s&imdb=%s&tmdb=%s' % (sys.argv[0], sysname, systitle, year, imdb, tmdb)
...
class libtvshows:
    ...
    def strmFile(self, i):
        try:
            ...
            content = '%s?action=playp&title=%s&year=%s&imdb=%s&tvdb=%s&season=%s&episode=%s&tvshowtitle=%s&date=%s' % (sys.argv[0], episodetitle, year, imdb, tvdb, season, episode, systitle, syspremiered)
