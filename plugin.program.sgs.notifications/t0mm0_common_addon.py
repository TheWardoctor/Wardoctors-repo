...
class Addon:
    ...
    def show_error_dialog(self, msg):###Modded by FTG###
        '''
    ...
    def show_small_popup(self, title='', msg='', delay=5000, image=''):
        ...
        '''
        xbmc.executebuiltin('XBMC.Notification("%s","%s",%d,"%s")' % (title, msg, delay, image))###Modded by FTG###
    def show_countdown(self, time_to_wait, title='', text=''):
        ...
        while secs <= time_to_wait:
            if (dialog.iscanceled()): cancelled = True; break###Modded by FTG###
    ...
    def resolve_url(self, stream_url):
        ...
        if stream_url:
            ...
            xbmcplugin.setResolvedUrl(self.handle, True, xbmcgui.ListItem(path=stream_url))###Modded by FTG###
        else:
    ...
    def get_music_playlist(self, new=False):
        ...
        Returns:
            A :class:`xbmc.Playlist` object.
       '''
        self.get_playlist(xbmc.PLAYLIST_MUSIC, new)###Modded by FTG###
    ...
    def add_item(self, queries, infolabels, contextmenu_items='', context_replace=False, img='', fanart='', resolved=False, total_items=0, playlist=False, item_type='video', is_folder=False):###Modded by FTG###
        '''
        ...
        Kwargs:
            ...
            'pictures')
        '''
        infolabels = self.unescape_dict(infolabels)###Modded by FTG###
        if not resolved:
        ...
        if playlist is not False:
            ...
            xbmcplugin.addDirectoryItem(self.handle, play, listitem, isFolder=is_folder, totalItems=total_items)###Modded by FTG###
    ...
    def _decode_callback(self, matches):
        ...
        id = matches.group(1)
        try: return unichr(int(id))###Modded by FTG###
        except: return id
    ...
    def unescape(self, text):
        ...
        try:
            ...
            for s, r in rep.items():
                text = text.replace(s, r)###Modded by FTG###
            # this has to be last:
            text = text.replace("&amp;", "&")
